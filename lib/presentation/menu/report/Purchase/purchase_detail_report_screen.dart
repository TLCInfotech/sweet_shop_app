...
import '../../transaction/purchase/create_purchase_activity.dart';
...
class _PurchaseDetailReportActivityState extends State<PurchaseDetailReportActivity> with CreatePurchaseInvoiceInterface{
  ...
  pdfDownloadCall(String urlType) async {
    ...
    if(netStatus==InternetConnectionStatus.connected){
      AppPreferences.getDeviceId().then((deviceId) async{
        ...
        String apiUrl ="";
        if (widget.come=="partyName") {
          apiUrl =baseurl + ApiConstants().getPurchasePartywise+"/Download?Company_ID=$companyId&${StringEn.lang}=$lang&From_Date=${DateFormat("yyyy-MM-dd").format(applicablefrom)}&To_Date=${DateFormat("yyyy-MM-dd").format(applicableTwofrom)}&Franchisee_ID=${widget.venderId}&Type=$urlType";
        } else if (widget.come=="itemName") {
          apiUrl =baseurl + ApiConstants().getPurchaseItemwise+"/Download?Company_ID=$companyId&${StringEn.lang}=$lang&From_Date=${DateFormat("yyyy-MM-dd").format(applicablefrom)}&To_Date=${DateFormat("yyyy-MM-dd").format(applicableTwofrom)}&Item_ID=${widget.itemId}&Type=$urlType";
        }
      print(apiUrl);
